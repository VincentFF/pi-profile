...
| 6 | `/profile use implement` | 无 reload 提示；footer 变为 `profile: implement`；`/profile status` 显示 `implement`、`beta-skill` 可见、tools 为 `[read, grep, find, ls]`；session 延续（`/session` 的 id 与文件不变） |
