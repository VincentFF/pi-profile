...
Verification: `tsc --noEmit` clean; `vitest run` 145/145 across 17 files, including real-pi integration: untrusted project catalogs/skills/extensions never enter the runtime (side-effect marker absent), trusted project gets selected `.pi` resources + merged settings with `defaultProjectTrust: "never"` intact, `--approve` grants one-run trust with filtering still applied and no persisted trust decision, `--no-approve` overrides a stored entry, project same-name replacement at launch, project-state restore.

**2026-09-12 — code review fixes**

Review (standards + spec, parallel agents) found and fixed: (1) the trust mirror dropped Pi's "no trust-requiring resources ⇒ trusted" step — now mirrored, with pi-profile-switch's own `.pi/profiles.json|resources.json|pi-profile-state.json` added to the trust-requiring set (a committed catalog/state file is pi-profile-switch's project attack surface); extension `project_trust` handlers stay unconsulted (would require executing extension code in the launcher) — documented as known divergence. (2) A trusted project's `packages` key leaked through the settings merge and would have installed into the symlinked global npm root — now stripped before merge. (3) A dangling *restored* profile (either scope) now warns and falls back to `default` instead of exit 2 — restore is a convenience; explicit positional selection still fails loudly. This revises ticket 02's "loud restore" decision. (4) Standards: `RuntimeStateStore` param renamed (`stateDir`, may be a project `.pi` dir), shared `isRecord` replaces a local duplicate, catalog entry type named, CLI rules doc amended with the default-profile `--approve` carve-out. Verified non-issues: Pi's `maybeAutoTrustAfterReload` trust write only fires when the session is already trusted (unreachable in named-profile sessions) — latent, documented; the TUI trust selector's writes into the ephemeral runtime trust.json vanish on exit (documented limitation of named-profile sessions).
