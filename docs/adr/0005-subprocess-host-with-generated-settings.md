...
- `PI_CODING_AGENT_DIR` points Pi at a pi-profile-switch-owned settings directory; `PI_CODING_AGENT_SESSION_DIR` keeps session storage in the real location. User configuration files are never modified.
...
- Known limitation: `pi install` / `pi config` inside a pi-profile-switch session write the generated settings and are lost on exit; use `/profile edit` or plain `pi` for persistent package changes.
