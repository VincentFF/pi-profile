...
Amended by [ADR-0005](0005-subprocess-host-with-generated-settings.md): the lock below stands, but the integration transport is no longer in-process host calls. pi-profile-switch runs inside Pi as an extension and coordinates with the adapter over the `pi.events` event bus; the profile-scoped state store contract for `/mcp enable|disable` is unchanged.
Amended again (ticket 10): the assumed adapter "profile-scoped state store" does not exist (pi-mcp-adapter@2.33.0 exposes only the register/snapshot event pair — no allowlist, no profile-state API, no enable/disable/reload event). The profile's `mcp` array in its owning catalog IS the profile-scoped persistent store; `/mcp enable|disable` edit it via ProfileCatalogStore and take effect through the standard rewrite-settings-and-reload path, with the allowlist republished at session_start over the coordination channel. The lock's intent stands: connection parameters, OAuth, and tokens remain adapter-managed; pi-profile-switch never writes them.
...
Considered alternative: pi-profile-switch manages MCP configuration itself. Rejected because it would fork server management away from the adapter's single implementation and pull credentials into profile catalogs.
